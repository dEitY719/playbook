...
|-------|-----------------|-----------|
| 1 | `ERR_SSL_PROTOCOL_ERROR` | 9988 포트가 nginx 443이 아닌 80(HTTP)에 매핑됨 |
| 2 | `MSIS9224`, `MSIS9221` | `redirect_uri` 불일치 + Docker Compose 중첩 변수 미확장 |
| 3 | `Invalid issuer` | issuer를 authorize URL에서 잘못 추론 |
...
다음에 또 SSO 디버깅을 하게 된다면, 에러 메시지보다 먼저 이 질문부터 할 것입니다.
